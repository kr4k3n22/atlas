...
└── lib/
    ...
    ├── emailService.ts           # Proton Mail SMTP approval notification (nodemailer + Bridge)
...
| `EMAIL_FROM` | ⬜ | Proton Mail sender address (e.g. `AtlasCaseActions@proton.me`) |
| `EMAIL_SMTP_PASSWORD` | ⬜ | Bridge-generated SMTP password for the sender account |
| `EMAIL_TO` | ⬜ | Recipient address for implementation-team approval notifications |
| `EMAIL_SMTP_HOST` | ⬜ | SMTP host for Proton Bridge (default: `127.0.0.1`) |
| `EMAIL_SMTP_PORT` | ⬜ | SMTP port for Proton Bridge (default: `1025`) |
| `EMAIL_SMTP_SECURE` | ⬜ | Set `true` for TLS on port 465; leave unset for STARTTLS (default: `false`) |
...
ATLAS sends from the shared Proton Mail account **AtlasCaseActions@proton.me** via the **Proton Mail Bridge** — a lightweight app that acts as a local SMTP relay, transparently handling Proton's end-to-end encryption.

> If you want to use a different sender address, replace `AtlasCaseActions@proton.me` with your own Proton Mail address in the environment variables below and sign in to Bridge with that account instead.

**1. Install Proton Mail Bridge**

Download and install Bridge for your OS from <https://proton.me/mail/bridge>.
Bridge must be running whenever ATLAS needs to send emails. For a server deployment, install it on the host that runs the ATLAS Next.js app (or on any machine reachable from it).

**2. Sign in to Bridge with the ATLAS Proton account**

Open Bridge and sign in with `AtlasCaseActions@proton.me` and the account password.

**3. Get the Bridge SMTP password**

Inside Bridge, click on the account row → **More details**. You will see:

| Setting | Value |
|---------|-------|
| SMTP Host | `127.0.0.1` |
| SMTP Port | `1025` |
| Username | `AtlasCaseActions@proton.me` |
| Password | _(Bridge-generated — copy this)_ |

> ⚠️ This password is generated by Bridge and is **not** your Proton account password. It changes each time Bridge is reinstalled.

**4. Set the environment variables**

Add these to your `.env.local` (local) or Vercel/deployment environment settings:

```env
# Proton Mail sender address
EMAIL_FROM=AtlasCaseActions@proton.me

# Bridge-generated SMTP password (from step 3)
EMAIL_SMTP_PASSWORD=<bridge-generated-password>

# Team inbox that should receive and act on approved cases
EMAIL_TO=implementation-team@yourorg.com

# Bridge SMTP connection — defaults below work when Bridge runs on the same machine.
# Change EMAIL_SMTP_HOST if Bridge runs on a separate server.
...
If you are deploying to Vercel, add all five variables via **Project → Settings → Environment Variables**, then trigger a new deployment so the server picks them up.

> **Note for Vercel deployments:** Vercel serverless functions cannot reach a Bridge instance running on your local machine (it is only reachable on `127.0.0.1`). For production, run Bridge on a VPS or cloud VM and set `EMAIL_SMTP_HOST` to its public/private IP address. Alternatively, use a standard SMTP relay (e.g. SendGrid, Mailgun) by pointing `EMAIL_SMTP_HOST/PORT` at their SMTP endpoint and setting `EMAIL_SMTP_PASSWORD` to their API key.
...
> **Tip:** If no email arrives, check your server logs for `[emailService]` warning lines. Common causes are: Bridge not running, wrong SMTP password, incorrect host/port, or missing environment variables.
...
- [x] **Email notifications on case approval** — Proton Mail SMTP (Bridge) notification to implementation team with full case details
