INSERT INTO app.claimant_case (
  ...
  jsonb_build_object(
    ...
    'structured_inputs', jsonb_build_object(
      'idv_status', 'verified',
      ...
      'docs_status', jsonb_build_object(
        'docs_requested', jsonb_build_array('termination_notice'),
        'docs_received', jsonb_build_array('termination_notice'),
        'docs_quality', 'valid'
      ),
      'engagement_barriers', jsonb_build_object(
    ...
    'harm_rights_signals', jsonb_build_object(
      'signal_level', 'none',
      'signal_type', jsonb_build_array(),
...
(
  ...
  jsonb_build_object(
    ...
    'structured_inputs', jsonb_build_object(
      ...
      'bank_data_access', 'consented',
      'docs_status', jsonb_build_object(
        'docs_requested', jsonb_build_array('termination_notice', 'employer_separation_certificate'),
        'docs_received', jsonb_build_array(),
    ...
    'harm_rights_signals', jsonb_build_object(
      'signal_level', 'high',
      'signal_type', jsonb_build_array('childcare_dependents', 'risk_of_homelessness'),
      'signal_source', 'system',
      'notes', 'Claimant reports imminent inability to pay for essentials (rent/food). High rights-impact / livelihood risk requires human oversight despite otherwise clear eligibility signals.'
    )
  )
),
(
  ...
  jsonb_build_object(
    ...
    'structured_inputs', jsonb_build_object(
      ...
      'bank_data_access', 'consented',
      'docs_status', jsonb_build_object(
        'docs_requested', jsonb_build_array('employment_history_statement'),
        'docs_received', jsonb_build_array('employment_history_statement'),
        'docs_quality', 'valid'
      ),
      'engagement_barriers', jsonb_build_object(
        'language_barrier', 'none',
    ...
    'free_text', jsonb_build_object(
      'claimant_message', 'My fixed-term contract ended and I was offered another contract, which I chose not to accept. I have insufficient contribution history. I am financially stable and not experiencing hardship. I am only seeking clarification on eligibility.',
      'agent_chat_transcript_excerpt', 'Fixed-term contract concluded. Renewal was offered and declined. Contribution history below statutory minimum threshold. All documentation verified.',
    ...
    'harm_rights_signals', jsonb_build_object(
      ...
      'signal_type', jsonb_build_array(),
      'signal_source', 'system',
      'notes', 'No indicators of livelihood risk, dependents, disability, or fundamental rights impact requiring human oversight.'
    )
  )
),
(
  ...
  jsonb_build_object(
    ...
    'structured_inputs', jsonb_build_object(
      ...
      'bank_data_access', 'consented',
      'docs_status', jsonb_build_object(
        'docs_requested', jsonb_build_array('employment_history_statement', 'separation_statement', 'final_payslip_or_earnings_record'),
        'docs_received', jsonb_build_array('employment_history_statement'),
    ...
    'free_text', jsonb_build_object(
      'claimant_message', 'I left my job last month and I''m currently not working. I''m not sure what you need from my employer, and I can upload whatever documents you require.',
      'agent_chat_transcript_excerpt', 'IDV and residency verified. Claimant reports unemployment after voluntary resignation. Contribution status and recent earnings cannot be confirmed from provided records. Employer report not yet received. Partial income verification; additional documents required to determine eligibility.',
      'caseworker_note', 'Do not escalate. Route to chatbot for targeted clarification and document collection; resubmit to MCP Brain after each response until statutory eligibility can be determined.'
    ),
    'harm_rights_signals', jsonb_build_object(
      'signal_level', 'none',
      'signal_type', jsonb_build_array(),
      'signal_source', 'system',
      'notes', 'No indicators of livelihood risk, dependents, disability, coercion, or other fundamental-rights impact requiring human oversight at this time. Remain in automated review loop unless risk signals emerge.'
    )
  )
);